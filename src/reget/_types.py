...
"""Zero-based index into ``PieceTracker``'s done/inflight bitfields."""
...
class DownloadComplete:
    """Download finished; all pieces are on disk.
...
class DownloadPartial:
    """Download stopped before all pieces were in hand.
...
"""Union of the two terminal outcomes returned by ``finalize()``."""
...
class ContentRangeError(RegetError):
    """Content-Range header doesn't match the expected piece boundaries."""


class HashBuilder:
    """Incremental SHA-256 accumulator fed during writes."""
    ...
    def update(self, offset: int, data: bytes) -> None:
        ...
        For a sequential stream (offset always == self._pos) this
        produces a correct rolling hash.  For out-of-order piece writes,
        hashing is deferred to a final pass after all pieces complete.
        """
        if offset == self._pos:
            self._h.update(data)
            self._pos += len(data)
    ...
    def sequential(self) -> bool:
        ...
        return True  # tracked externally; kept for API symmetry
